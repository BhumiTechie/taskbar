...
const App = () => {
  ...
  //     isVaild = confirm(" Do you really want to delete this task?");
  //  }
  //  if(isVaild || copyTasks[i].completed){
  ...
  if (tasks.length > 0) {
    taskRender = tasks.map((task, index) => (
      <li className="flex items-center justify-between bg-gray-800 p-4 rounded-md mb-2"
        key={index}>
        <div className="flex items-center">

          <div
            onClick={(e) => CompleteTaskToggle(e, index)}
            className={`${task.completed ? "bg-green-500" : "border" }  rounded-full w-[3vmax] h-[3vmax] bg-red-300 border-orange-600 mr-1`}

          >
            
            {/* {tasks.completed ? <i className="ri-check-line text-white"></i> : null} */}
          </div>
          <h1
